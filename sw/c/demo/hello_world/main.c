...
#define BTNL_MASK ((SW0_MASK << 7) << 1)
#define BTNR_MASK ((SW0_MASK << 7) << 2)
#define BTNU_MASK ((SW0_MASK << 7) << 3)
#define BTND_MASK ((SW0_MASK << 7) << 4)
#define BTNC_MASK ((SW0_MASK << 7) << 5)
...
int main(void) {
  ...
  while (1) {
    ...
    if (cur_time != last_elapsed_time) {
      ...
      // Cycle through LEDs unless BTNC is pressed
      // uint32_t out_val = read_gpio(GPIO_OUT);
      // out_val = (out_val << 1) & 0xF;
      // if ((in_val & BTNC_MASK) || (out_val == 0)) {
      //   out_val = LED(0);
      // }
      ...
      uint32_t out_val = read_gpio(GPIO_OUT);
      if (in_val & BTNL_MASK)
        out_val = out_val & LED(7) ? LED(0) : out_val << 1;
      else if (in_val & BTNR_MASK)
        out_val = out_val & LED(0) ? LED(7) : out_val >> 1;
